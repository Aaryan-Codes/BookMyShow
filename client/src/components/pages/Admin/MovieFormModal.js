...
import { addMovie,updateMovie } from '../../../apicalls/movieAPIcall';
import moment from 'moment';

const MovieFormModal = ({isModalOpen, setIsModalOpen, formType, selectedMovie, setSelectedMovie, getData}) => {
        ...
        <Modal centered title={formType === "add" ? "Add Movie" : "Edit Movie"} open={isModalOpen} onCancel={handleCancel} width={800} footer={null} >
          <Form layout='vertical' style={{width: "100%"}} initialValues={selectedMovie} onFinish={onFinish}>
            <Row gutter={{
              ...
              <Col span={24}>
                <Form.Item label="Movie Name" htmlFor='title' name="title" className='d-block' rules={[{required: true, message: "Movie name is required!"}]}>
                  <Input id="title" type="text" placeholder='Enter the movie name'></Input>
                </Form.Item>
              </Col>
              <Col span={24}>
                <Form.Item label="Description" htmlFor='description' name="description" className='d-block' rules={[{required: true, message: "Description is required!"}]}>
                  <TextArea id="description" rows="4" placeholder='Enter the  description'></TextArea>
                </Form.Item>
              </Col>
              <Col span={24}>
                <Row  gutter={{
                ...
                <Col span={8}>
                    <Form.Item  label="Movie  Duration (in min)" htmlFor='duration' name="duration" className='d-block' rules={[{required: true, message: "Movie duration  is required!"}]}>
                    <Input id="duration" type="number" placeholder='Enter the movie duration'></Input>
                  </Form.Item>
                </Col>
                <Col span={8}>
                ...
                <Col span={8}>
                  <Form.Item  label="Release Date" htmlFor='releaseDate' name="releaseDate" className='d-block' rules={[{required: true, message: "Movie Release Date is required!"}]}>
                  <Input id="releaseDate" type="date" placeholder='Choose the release date'></Input>
                </Form.Item>
                </Col>
              </Row>
              </Col>
